...
import eu.nazgee.wallpaper.solar.LightConverter.LightFeedback;
...
//			Log.d(getClass().getSimpleName(), "avg=" + avg);
		...
		if (mTimePassedLong >= 5) {
			for (int i = 0; i < mCells.length; i++) {
				Cell cell = mCells[i];
				
				final float timeOffset = (float)i/(float)mCells.length * mTimePassedLong/5 + 0.1f;
